...
SEXP CompleteCaseImputation(SEXP Rseries, SEXP Rdimension, SEXP Rdelay, SEXP Rpatterns, SEXP Relements, SEXP Rsymbols){
	    ...
	    int dimFat = 1, n, i, j, duplicated_elements = 0, seriesize, dimension, delay, aux = 0;
		int* symbols_aux;
		int* patterns_aux;
		double* elements_aux;
	    SEXP Rprobability;
	    ...
	    for(i = 0; i < dimFat; i++){
	        REAL(Rprobability)[i] = REAL(Rprobability)[i]/((seriesize - (dimension - 1)*delay) - duplicated_elements);
	    }
