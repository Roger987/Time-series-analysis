...
Entropy.complexity.values <- function(Series.to.analysis, dimension, delay){
  ...
  for(i in c(1:dim(Series.to.analysis)[1])){
    ...
    prob[1,] <- completeCase(Series.to.analysis[1,], dimension, delay, patterns, elements, symbols)
    #Entropy[i] <- shannonNormalized(prob[i,])
    #Complexity[i] <- Ccomplexity(prob[i,])
    
    start.time <- Sys.time()
    prob1[1,] <- dataDriven(Series.to.analysis[1,], dimension, delay, patterns, elements, symbols, prob[1,]) #Green
    Entropy1[1] <- shannonNormalized(prob1[1,])
    Complexity1[1] <- Ccomplexity(prob1[1,])
    end.time <- Sys.time()
    time.taken <- end.time - start.time
    
    dyn.load("DataDrivenImputation.so")
    
    start.time <- Sys.time()
    prob2[1,] <- .Call("DataDrivenImputation", Series.to.analysis[1,], dimension, delay, as.integer(as.vector(t(patterns))), as.vector(t(elements)), as.integer(as.vector(t(symbols))), prob[1,])
