...
Entropy.complexity.values <- function(Series.to.analysis, dimension, delay){
  ...
  for(i in c(1:dim(Series.to.analysis)[1])){
    ...
    start.time <- Sys.time()
    prob1[1,] <- completeCase(Series.to.analysis[1,], dimension, delay, patterns, elements, symbols) #Blue
    ...
    start.time <- Sys.time()
    prob2[1,] <- .Call("CompleteCaseImputation", Series.to.analysis[1,], dimension, delay, as.integer(as.vector(t(patterns))), as.vector(t(elements)), as.integer(as.vector(t(symbols))))
